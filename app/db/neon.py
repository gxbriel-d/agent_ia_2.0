...
def get_db_connection():
    """
    Retorna uma conexão com o banco Neon PostgreSQL.
    """
...
def execute_query(query: str, params: tuple = None, fetch_all: bool = True):
    """
    Executa uma query SELECT no Neon PostgreSQL e retorna os resultados como dicionários.
    """
    conn = None
    try:
...
def execute_write(query: str, params: tuple = None) -> bool:
    """
    Executa uma query INSERT / UPDATE / DELETE no Neon PostgreSQL com commit automático.
    """
    conn = None
    try:
        conn = get_db_connection()
